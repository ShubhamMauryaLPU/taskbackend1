...
const setStatus = async () => {
  try {
    ...
    for (const item of allGst) {
      item.status = item.returns?.[0]?.status || "pending";
      await item.save();
    }
    console.log("GST statuses updated successfully");
  } catch (err) {
    console.error("Error updating GST statuses:", err);
  }
};
